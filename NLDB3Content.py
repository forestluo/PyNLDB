...
class NLDB3Raw (NLDB3) :
    ...
    def total(self) :
        ...
        # 设置SQL语句
        dbSQL = "SELECT COUNT(*) as count FROM " + NLDB3Raw._tableName
        # 执行语句
        self._dbCursor.execute(dbSQL)
        # 获得返回数据
        dataResult = self._dbCursor.fetchone()
    ...
    def random(self) :
        ...
        # 设置SQL语句
        dbSQL = "SELECT TOP 1 source,content FROM " + NLDB3Raw._tableName + " ORDER BY NEWID()"
        # 执行语句
        self._dbCursor.execute(dbSQL)
        # 获得返回数据
        dataResult = self._dbCursor.fetchone()
...
class NLDB3Dictionary(NLDB3) :
    ...
    def total(self):
        ...
        # 设置SQL语句
        dbSQL = "SELECT COUNT(*) as count FROM " + NLDB3Dictionary._tableName
        # 执行语句
        self._dbCursor.execute(dbSQL)
        # 获得返回数据
        dataResult = self._dbCursor.fetchone()
    ...
    def traverse(self, function):
        ...
        total = self.total()
        # 打印数据总数
        print("NLDB3Dictionary.traverse : try to process %d row(s) !" % total)

        # 百分之一
        percent = 0
        onePercent = total / 100.0
        # 设置SQL语句
        dbSQL = "SELECT * FROM " + NLDB3Dictionary._tableName
        # 执行语句
        self._dbCursor.execute(dbSQL)
        # 获得返回数据
        dataResult = self._dbCursor.fetchone()
        ...
        # 打印数据总数
        print("")
        print("NLDB3Dictionary.traverse : %d row(s) processed !" % total)
    ...
    def save(self, fileName):
        ...
        jsonFile = open(fileName, "w", encoding="utf-8")
        # 打印信息
        print("NLDB3Dictionary.save : file(\"%s\") opened !" % fileName)
        # 检查文件
        assert jsonFile is not None
        ...
        total = self.total()
        # 打印数据总数
        print("NLDB3Dictionary.save : try to save %d row(s) !" % total)
        # 将总数写入文件
        jsonFile.write(str(total))
        jsonFile.write("\n")
        ...
        # 打印数据总数
        print("")
        print("NLDB3Dictionary.save : %d row(s) saved !" % total)
        # 关闭文件
        jsonFile.close()
        # 打印信息
        print("NLDB3Dictionary.save : file(\"%s\") closed !" % fileName)
