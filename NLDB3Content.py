...
class NLDB3Raw (NLDB3) :
    ...
    def save(self, file_name) :
        # 调用父类函数
        self._save("SELECT source, content FROM " + NLDB3Raw._tableName, file_name)

    def traverse(self, function) :
        # 调用父类函数
        self._traverse("SELECT source, content FROM " + NLDB3Raw._tableName, function)
...
class NLDB3Dictionary(NLDB3) :
    ...
    def save(self, file_name):
        # 调用父类函数
        self._save("SELECT count, source, content, remark FROM " + NLDB3Dictionary._tableName, file_name)

    def traverse(self, function):
        # 调用父类函数
        self._traverse("SELECT count, source, content, remark FROM " + NLDB3Dictionary._tableName, function)

class NLDB3Core(NLDB3) :
    # SQL语句
    __sql = "SELECT [content], [count] FROM [dbo].[DictionaryContent] " \
    ...
    def save(self, file_name):
        # 调用父类函数
        self._save(NLDB3Core.__sql, file_name)

    def traverse(self, function):
        # 调用父类函数
        self._traverse(NLDB3Core.__sql, function)

    def total(self):
        # 返回数据结果
        return self._total("SELECT COUNT(*) AS count FROM (" + NLDB3Core.__sql + ")")
